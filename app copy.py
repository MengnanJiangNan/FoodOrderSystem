        ...
        if 'orders' not in sheet_dict:
            ...
            if all(col in orders_df.columns for col in ['price', 'quantity', 'subtotal', 'food_id']):
                try:
                    ...
                    price_map = {}
                    for _, row in menu_df.iterrows():
                        price_map[int(row['id'])] = float(row['price'])
                    ...
                    for idx, row in orders_df.iterrows():
                        ...
                        if (pd.isna(row['subtotal']) or row['subtotal'] == 0) and price > 0 and quantity > 0:
                            orders_df.at[idx, 'subtotal'] = price * quantity
...
def load_menu_data():
    try:
        ...
        sample_data = {
            'id': [1, 2, 3, 4, 5],
            'name': ['汉堡包', '披萨', '沙拉', '可乐', '奶茶'],
            'description': ['经典牛肉汉堡', '芝士披萨', '田园沙拉', '可口可乐', '珍珠奶茶'],
            'price': [25.00, 48.00, 18.00, 6.00, 12.00],
            'category': ['主食', '主食', '小吃', '饮料', '饮料'],
            'image_url': ['burger.jpg', 'pizza.jpg', 'salad.jpg', 'cola.jpg', 'milktea.jpg']
        }
...
def load_orders():
    try:
        # 尝试从Excel文件加载订单数据
        orders_df = pd.read_excel('food_orders.xlsx')
        return orders_df
    except Exception as e:
        # 如果加载失败，返回空DataFrame
        print(f"Error loading orders: {e}")
        return pd.DataFrame(columns=['id', 'customer_name', 'items', 'total_price', 'status', 'timestamp'])

# 创建模拟订单函数
def place_order(name, phone, address, items, special_requests):
    if not name or not phone or not items:
        return "订单失败: 请填写姓名、电话，并选择至少一个菜品"
    
    items_str = ", ".join(items)
    order_details = f"订单已成功提交!\n\n客户: {name}\n电话: {phone}\n地址: {address}\n菜品: {items_str}"
...
# 创建Gradio界面
def create_food_order_interface():
    menu_data = load_menu_data()
    
    with gr.Blocks(title="食品订购系统") as demo:
        gr.Markdown("# 🍔 食品订购系统")
        gr.Markdown("欢迎使用我们的在线食品订购平台")
        
        with gr.Tab("菜单浏览"):
            with gr.Row():
                category_filter = gr.Dropdown(
                    choices=["全部"] + list(menu_data['category'].unique()),
                    value="全部",
                    label="分类过滤"
                )
            ...
            def filter_menu(category):
                if category == "全部":
                    ...
                    filtered_data = menu_data[menu_data['category'] == category]
                
                # 格式化为显示数据
                display_data = filtered_data[['name', 'description', 'price', 'category']].copy()
                display_data['price'] = display_data['price'].apply(lambda x: f"¥{x:.2f}")
                return display_data
            
            menu_display = gr.Dataframe(
                headers=["菜品名称", "描述", "价格", "分类"],
                datatype=["str", "str", "str", "str"],
                value=filter_menu("全部"),
                label="菜单"
            )
            
            category_filter.change(filter_menu, inputs=[category_filter], outputs=[menu_display])
        ...
        with gr.Tab("下单"):
            with gr.Row():
                with gr.Column():
                    ...
                    address = gr.Textbox(label="送餐地址")
                
                with gr.Column():
                    # 获取所有菜品名称作为选项
                    all_items = list(menu_data['name'])
                    order_items = gr.CheckboxGroup(
                        choices=all_items,
                        label="选择菜品"
                    )
                    
                    special_requests = gr.Textbox(
                        label="特殊要求",
                        placeholder="例如：少放辣、不要洋葱等"
                    )
            
            submit_btn = gr.Button("提交订单", variant="primary")
            order_result = gr.Textbox(label="订单结果", lines=8)
            
            submit_btn.click(
                place_order,
                inputs=[name, phone, address, order_items, special_requests],
        ...
        with gr.Tab("关于"):
            ...
            这是一个基于Flask和Gradio的食品订购系统，允许用户浏览菜单并下单。
...
# 创建并运行Gradio界面
demo = create_food_order_interface()
